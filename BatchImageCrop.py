...
if __name__ == "__main__":
    ...
    def build_ui(self):
        ...
        # 图片列表
        ttkb.Label(right_panel, text="图片列表", font=("", 10, "bold")).pack(pady=(5, 2))
        self.count_label = ttkb.Label(right_panel, text="共 0 张", bootstyle="secondary")
        self.count_label.pack()
        ...
        # 变换面板（每张独立）
        ttkb.Separator(right_panel).pack(fill=X, pady=6)
        ttkb.Label(right_panel, text="当前图片变换", font=("", 10, "bold")).pack()
        ttkb.Label(right_panel, text="（先变换，再裁剪）", bootstyle="secondary").pack()
        ...
        self.info_label = ttkb.Label(bot_frame, text="就绪 — 添加图片后拖拽裁剪框调整位置", font=("", 9))
        self.info_label.pack(side=LEFT)

        self.progress = ttkb.Progressbar(bot_frame, mode="determinate", length=180, bootstyle="info")
        self.progress.pack(side=LEFT, padx=15)
        self.progress_label = ttkb.Label(bot_frame, text="", font=("", 9))
        self.progress_label.pack(side=LEFT)
    ...
    def apply_ratio(self):
        try:
            ...
            if w <= 0 or h <= 0:
                raise ValueError
            # 简化比例
            g = math.gcd(int(w * 1000), int(h * 1000))
            self.ratio_w = w
            self.ratio_h = h
            # 重新计算裁剪框
            self._recalc_crop_box()
    ...
    def batch_crop(self):
        ...
        for i, path in enumerate(self.image_files):
            try:
                ...
                # 第一张图用用户调整的位置，其他图用相同比例位置
                if i == self.current_index or i == 0:
                    cl = self.crop_left
                    ct = self.crop_top
                else:
                    cl = self.crop_left
                ...
                cl = max(0, min(cl, 1.0 - crop_w_ratio))
                ct = max(0, min(ct, 1.0 - crop_h_ratio))
